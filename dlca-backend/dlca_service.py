...
def dlca_run_from_form():
    ...
    try:
        ...
        charts = raw_result.get("all_charts_data") or {}
        building_id = payload.get("building_id", "frontend_building_1")
        building_type = payload.get("building_type", "Masonry_3")
        cumulative = payload.get("cumulative", "cumulative")
        scenario = payload.get("dynamic_scenario", "Carbon Neutral")

        totals = charts.get("totals", {})
        energy = charts.get("energy", {})
        comps  = charts.get("components", {})
        waste  = charts.get("waste", {})

        groups = []
        grand = totals.get("grand_total") or totals.get("base_and_waste") or totals.get("base_materials") or []
        g1 = {"name": "1. Total Lifecycle (incl. Waste + B6)", "total_line": grand, "sub_lines": {}, "pdf": None}
        if totals.get("base_and_waste"):
            g1["sub_lines"]["Materials incl. waste"] = totals["base_and_waste"]
        elif totals.get("base_materials"):
            g1["sub_lines"]["Materials"] = totals["base_materials"]
        if energy.get("Heat"):  g1["sub_lines"]["Heating (B6)"] = energy["Heat"]
        if energy.get("Power"): g1["sub_lines"]["Power (B6)"] = energy["Power"]
        groups.append(g1)

        g2 = {"name": "2. Building Components Summary",
              "total_line": totals.get("base_materials") or [],
              "sub_lines": {str(k): v for k, v in comps.items()}, "pdf": None}
        groups.append(g2)

        for k, v in comps.items():
            groups.append({"name": f"3. Breakdown: {building_type}_{k}",
                           "total_line": v, "sub_lines": {}, "pdf": None})

        if waste:
            groups.append({"name": "4. Waste (end-of-life)",
                           "total_line": totals.get("waste_total") or [],
                           "sub_lines": {str(k): v for k, v in waste.items()}, "pdf": None})

        # Attach PDFs generated by THIS run only: filter by building id, the exact
        # cumulative token and the scenario, so runs with other settings never mix in.
        try:
            plots_dir = Path(dlca_core.PLOTS_DIR) if hasattr(dlca_core, "PLOTS_DIR") else Path(dlca_core.__file__).parent / "results" / "plots"
            cum_token = f"_{cumulative}"
            for pdf_path in glob.glob(str(plots_dir / "with_waste" / f"*{building_id}*.pdf")):
                fn = os.path.basename(pdf_path)
                if cum_token not in fn: continue
                if cumulative == "cumulative" and "_non-cumulative" in fn: continue
                if scenario not in fn and "_-" not in fn: continue
                with open(pdf_path, "rb") as fh:
                    b64 = f"data:application/pdf;base64,{base64.b64encode(fh.read()).decode()}"
                if "subparts" in fn and "including waste" in fn:
                    if not g1["pdf"]: g1["pdf"] = b64
                elif "subparts" in fn and not any(c in fn for c in comps):
                    if not g2["pdf"]: g2["pdf"] = b64
                else:
                    for g in groups[2:]:
                        comp_key = g["name"].rsplit("_", 1)[-1] if g["name"].startswith("3.") else None
                        if comp_key and comp_key in fn and not g["pdf"]:
                            g["pdf"] = b64
                            break
        except Exception as _e:
            print("[run_from_form] pdf attach skipped:", _e)

        total_impact = float(grand[-1]) if grand else 0.0
        return jsonify({"ok": True, "result": {"total_impact": total_impact, "groups": groups}})
